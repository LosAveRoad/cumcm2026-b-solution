...
N_Q3_MAX = 16
# Q4 occupancy in the simulator matches that 10–16 band (live 12/13/16).
N_Q4_MAX = 16
...
Q4_SHIFT_A = 500.0
Q4_LATTICE_S = 850.0
Q4_RHO = Q4_LATTICE_S / math.sqrt(3.0)
Q4_Q_DISK_R = ARENA_R + Q4_SHIFT_A
# Pose S hears (g,u) iff ||S-g||≤1000 and (S-g)·u≥0. Conservative q-hit:
# disk(S, a) in q-space. Lattice p is lemma-covered by a scan within a-ρ.
Q4_LEMMA_NEAR = Q4_SHIFT_A - Q4_RHO  # ≈9.25 m
_Q4_SE2_CACHE: tuple[list[tuple[float, float]], list[tuple]] | None = None
...
def _pose_hears_gu(
    ...
    dx = s[0] - g[0]
    dy = s[1] - g[1]
    if dx * dx + dy * dy > R_HEAR * R_HEAR + 1e-6:
        return False
    return dx * u[0] + dy * u[1] >= -1e-9


def _q4_se2_cache() -> tuple[list[tuple[float, float]], list[tuple]]:
    """Finite (g,u) hitting-set on D(0,1800)×S¹. Skip is not this grid;

    the ρ<500 lattice is the P=1 shield. Samples only rank / drop cities
    whose exclusive leftover is empty, then fallback restores unheard cells.
    """
    global _Q4_SE2_CACHE
    if _Q4_SE2_CACHE is not None:
        return _Q4_SE2_CACHE
    lattice = q4_scouts()
    headings: list[tuple[float, float]] = []
    n_head = 24
    for k in range(n_head):
        ang = 2.0 * math.pi * k / n_head
        headings.append((math.cos(ang), math.sin(ang)))
    gs: list[tuple[float, float]] = [(0.0, 0.0)]
    dr = 90.0
    n_ring = int(math.ceil(ARENA_R / dr))
    for i in range(1, n_ring + 1):
        r = min(ARENA_R, dr * i)
        n_ang = max(8, int(round(2.0 * math.pi * r / dr)))
        for k in range(n_ang):
            ang = 2.0 * math.pi * k / n_ang
            gs.append((r * math.cos(ang), r * math.sin(ang)))
    samples: list[tuple] = []
    rho = Q4_RHO
    for g in gs:
        for u in headings:
            q = (g[0] + Q4_SHIFT_A * u[0], g[1] + Q4_SHIFT_A * u[1])
            certs: list[int] = []
            for i, p in enumerate(lattice):
                if hypot(p, q) <= rho + 1e-6:
                    certs.append(i)
            samples.append((g, u, q, tuple(certs)))
    _Q4_SE2_CACHE = (lattice, samples)
    return _Q4_SE2_CACHE
...
class Searcher:
    ...
    _q4_visited: list[tuple[float, float]] = field(default_factory=list)
    _q4_leftover: list[tuple] | None = None
    _q4_lattice_index: dict[tuple[float, float], int] | None = None
    ...
    def _q4_idx(self, scout: tuple[float, float]) -> int:
        if self._q4_lattice_index is None:
            self._q4_lattice_index = {}
            for i, p in enumerate(q4_scouts()):
                self._q4_lattice_index[(round(p[0], 6), round(p[1], 6))] = i
        key = (round(scout[0], 6), round(scout[1], 6))
        hit = self._q4_lattice_index.get(key)
        if hit is not None:
            return hit
        lattice = q4_scouts()
        return min(range(len(lattice)), key=lambda i: hypot(lattice[i], scout))

    def _q4_scout_done(self, scout: tuple[float, float]) -> bool:
        if any(hypot(scout, v) <= 25.0 for v in self._q4_visited):
            return True
        # Scan within a−ρ of p lemma-covers disk(p, ρ) in q-space.
        return any(hypot(scout, d) <= Q4_LEMMA_NEAR + 0.5 for d in self.discovery_stations)

    def _q4_leftover_samples(self) -> list[tuple]:
        if self._q4_leftover is not None:
            return self._q4_leftover
        _lattice, samples = _q4_se2_cache()
        stations = self.discovery_stations
        leftover: list[tuple] = []
        for g, u, q, certs in samples:
            heard = False
            for s in stations:
                if _pose_hears_gu(s, g, u):
                    heard = True
                    break
            if not heard:
                leftover.append((g, u, q, certs))
        self._q4_leftover = leftover
        return leftover

    def _q4_se2_residual_nonempty(self) -> bool:
        if self._n_found() >= N_Q4_MAX:
            return False
        if self._q4_leftover_samples():
            return True
        for scout in q4_scouts():
            if self._q4_scout_done(scout):
                continue
            if self._q4_cell_unheard(scout):
                return True
        return False

    def _q4_cell_unheard(self, p: tuple[float, float]) -> bool:
        """Heading-slice samples of p's ρ-disk. Used as P=1 shield, not skip."""
        if self._q4_scout_done(p):
            return False
        stations = self.discovery_stations
        if not stations:
            return True
        n_u = 48
        rings = (0.45 * Q4_RHO, 0.85 * Q4_RHO)
        for k in range(n_u):
            ang = 2.0 * math.pi * k / n_u
            ux, uy = math.cos(ang), math.sin(ang)
            cx = p[0] - Q4_SHIFT_A * ux
            cy = p[1] - Q4_SHIFT_A * uy
            pts = [(cx, cy)]
            for rr in rings:
                n_b = 12 if rr > 0.6 * Q4_RHO else 8
                for j in range(n_b):
                    a2 = 2.0 * math.pi * j / n_b
                    pts.append((cx + rr * math.cos(a2), cy + rr * math.sin(a2)))
            for g in pts:
                if hypot(g) > ARENA_R + 1e-6:
                    continue
                u = (ux, uy)
                if not any(_pose_hears_gu(s, g, u) for s in stations):
                    return True
        return False

    def _q4_scout_has_leftover(self, scout: tuple[float, float], leftover: list[tuple]) -> bool:
        idx = self._q4_idx(scout)
        lattice = q4_scouts()
        for _g, _u, _q, certs in leftover:
            if idx not in certs:
                continue
            other_done = False
            for j in certs:
                if j == idx:
                    continue
                if self._q4_scout_done(lattice[j]):
                    other_done = True
                    break
            if not other_done:
                return True
    ...
    def _q4_prune_scouts(self) -> None:
        """Drop a lattice city when its exclusive leftover (g,u) is empty.

        Continuum skip on the q-disk is the lemma-near test and the heading
        slice fallback, not a heading leftover count. Empty samples do not
        finish the net: `_q4_certificate_fallback` restores unheard cells.
        """
        ...
        for scout in self.remaining_scouts:
            if self._q4_scout_done(scout):
                continue
            if leftover and self._q4_scout_has_leftover(scout, leftover):
                kept.append(scout)
        self.remaining_scouts = kept

    def _q4_certificate_fallback(self) -> None:
        if not self._silent_channels() or self._n_found() >= N_Q4_MAX:
            return
        leftover = self._q4_leftover_samples()
        have = list(self.remaining_scouts)
        for scout in q4_scouts():
            ...
            if self._q4_scout_done(scout):
                continue
            if leftover:
                if self._q4_scout_has_leftover(scout, leftover):
                    self.remaining_scouts.append(scout)
                continue
            if self._q4_cell_unheard(scout):
                self.remaining_scouts.append(scout)

    def _q4_pose_hits_leftover(self, pt: tuple[float, float]) -> bool:
        leftover = self._q4_leftover_samples()
        if not leftover:
            return False
    ...
    def _q4_n_leftover_hit(self, pt: tuple[float, float]) -> int:
        n = 0
        for g, u, _q, _c in self._q4_leftover_samples():
            if _pose_hears_gu(pt, g, u):
                n += 1
        return n

    def _q4_n_scouts_cleared_by(self, pt: tuple[float, float]) -> int:
        leftover = self._q4_leftover_samples()
        if not leftover:
            return 0
        n = 0
        for scout in self.remaining_scouts:
            rows = self._q4_scout_leftover_rows(scout, leftover)
            if rows and all(_pose_hears_gu(pt, g, u) for g, u, _q, _c in rows):
                n += 1
        return n

    def _q4_scout_leftover_rows(
        self, scout: tuple[float, float], leftover: list[tuple]
    ) -> list[tuple]:
        idx = self._q4_idx(scout)
        lattice = q4_scouts()
        rows: list[tuple] = []
        for row in leftover:
            ...
            if idx not in certs:
                continue
            if any(self._q4_scout_done(lattice[j]) for j in certs if j != idx):
                continue
            rows.append(row)
        return rows

    def _q4_covering_waypoint(self, scout: tuple[float, float]) -> tuple[float, float]:
        """First here→scout pose that hits this city's leftover samples.

        Scout centre is the certified fallback (a±ρ lemma).
        """
        here = (self.x, self.y)
        leftover = self._q4_leftover_samples()
        rows = self._q4_scout_leftover_rows(scout, leftover)
        if not rows:
            return scout
        if all(_pose_hears_gu(here, g, u) for g, u, _q, _c in rows):
            return here
        if hypot(here, scout) < 1e-9:
            return scout
        # On-the-way samples, G-hats, current pose as GTSP candidates.
        cands: list[tuple[float, float]] = [scout]
        for i in range(20):
            t = i / 19.0
            cands.append(
        ...
        for ch in list(self.fixes.keys()):
            ...
            if len(dirs) == 1:
                u = bearing_vec(float(dirs[0].svd_deg))
                s = (dirs[0].x, dirs[0].y)
                for r in (400.0, 550.0, 750.0):
                    cands.append(clip_to_arena((s[0] + r * u[0], s[1] + r * u[1])))
        best = scout
        best_d = hypot(scout, here)
        seen: list[tuple[float, float]] = []
        for p in cands:
            if any(hypot(p, q) <= 8.0 for q in seen):
                continue
            seen.append(p)
            if not all(_pose_hears_gu(p, g, u) for g, u, _q, _c in rows):
                continue
            d = hypot(p, here)
            if d < best_d - 1e-9:
                best_d = d
    ...
    def _q4_next_covering_pose(
        self,
    ) -> tuple[tuple[float, float], tuple[float, float] | None] | None:
        """Greedy disk-cover then open TSP on remaining ρ-disks."""
        here = (self.x, self.y)
        leftover = self._q4_leftover_samples()
        if here and leftover and self._q4_pose_hits_leftover(here):
            if not any(hypot(here, d) <= 30.0 for d in self.discovery_stations):
                return here, None
        if not self.remaining_scouts:
            return None
        wps: list[tuple[float, float]] = []
        ...
        for scout in self.remaining_scouts:
            wp = self._q4_covering_waypoint(scout)
            if any(hypot(wp, prev) <= 25.0 for prev in wps):
                continue
            wps.append(wp)
            meta.append(scout)
        if not wps:
            return None
        # Greedy: prefer a pose that finishes more leftover cities per walk.
        best_i = 0
        best_score = -1e18
        for i, wp in enumerate(wps):
            n_done = self._q4_n_scouts_cleared_by(wp)
            n_hit = self._q4_n_leftover_hit(wp)
            walk = hypot(wp, here)
            score = 900.0 * n_done + 0.08 * n_hit - walk
            if score > best_score:
                best_score = score
                best_i = i
        # Open TSP of the covering waypoints; take first if it is not a star.
        order = tsp_visit_order(here, wps)
        tsp0 = order[0]
        extra = hypot(wps[best_i], here) - hypot(wps[tsp0], here)
        if extra > 420.0 and best_score < 900.0:
            chosen = tsp0
        else:
            chosen = best_i
            # If TSP-first is almost as covering-rich and much closer, take it.
            if extra > 80.0:
                n_tsp = self._q4_n_scouts_cleared_by(wps[tsp0])
                if n_tsp >= self._q4_n_scouts_cleared_by(wps[best_i]) and hypot(
                    wps[tsp0], here
                ) + 40.0 < hypot(wps[best_i], here):
                    chosen = tsp0
        return wps[chosen], meta[chosen]

    def _q4_mark_visited_near(self, pt: tuple[float, float]) -> None:
        for scout in list(self.remaining_scouts):
            if hypot(pt, scout) <= 25.0:
                if not any(hypot(scout, v) <= 1e-6 for v in self._q4_visited):
                    self._q4_visited.append(scout)

    def _q4_batch_scan(self, pt: tuple[float, float], *, lattice: bool = False) -> None:
        """Silent leftover channels at one covering pose. No interrupt-chase."""
        silent = self._silent_channels()
        if not silent:
            return
        if any(hypot(pt, d) <= 20.0 for d in self.discovery_stations):
            self._q4_mark_visited_near(pt)
            return
        if (
            self.discovery_stations
            and not lattice
            and not self._q4_pose_hits_leftover(pt)
            and not self._q4_se2_residual_nonempty()
        ):
            return
        self.scan_point(pt, silent, chase=False)
        self._mark_discovery(pt)
    ...
    def _q4_path_sensor_scan(self) -> None:
        """Silent-scan at G/chase pose (5 s, no extra walk) iff residual remains."""
        if self._too_many():
            return
        silent = self._silent_channels()
        if not silent:
            return
        here = (self.x, self.y)
        if any(hypot(here, d) <= 25.0 for d in self.discovery_stations):
            return
        if not self._q4_se2_residual_nonempty():
            return
        if not self._q4_pose_hits_leftover(here) and self.discovery_stations:
            return
        self._q4_batch_scan(here, lattice=False)

    def _q4_ghat(self, channel: int) -> tuple[float, float] | None:
        post = self._posterior_centroid(channel)
        if post is not None:
            return post
        dirs = [
            fx
        ...
        if not dirs:
            near = [fx for fx in self.fixes.get(channel, []) if fx.kind == "near"]
            if near:
                return (near[-1].x, near[-1].y)
            return None
        fx = dirs[-1]
    ...
    def _q4_chase_now(self, channel: int, next_cover: tuple[float, float] | None) -> bool:
        """Insert a found G only if it is nearby, on-the-way, or hits leftover.

        Far sources wait for the end service TSP. Batch-scan already finished
        the covering pose, so a chase never walks back to that scout.
        """
        if not self.remaining_scouts or not self._q4_se2_residual_nonempty():
            return True
        ghat = self._q4_ghat(channel)
        if ghat is None:
            return True
        here = (self.x, self.y)
        dg = hypot(ghat, here)
        if dg <= NEARBY_CERT_M + 80.0:
            return True
        if next_cover is None:
            return True
        extra = dg + hypot(ghat, next_cover) - hypot(here, next_cover)
        if extra <= ON_WAY_EXTRA + 40.0:
            return True
        if self._q4_n_scouts_cleared_by(ghat) >= 1 and extra <= 520.0:
            return True
        return False

    def _q4_chase_useful(self, next_cover: tuple[float, float] | None) -> None:
        pending = [
            c
            for c in list(self.fixes.keys())
        ...
        here = (self.x, self.y)
        pending.sort(
            key=lambda c: hypot(self._q4_ghat(c) or here, here)
        )
        for ch in pending:
            if self._too_many() or ch in self.cleared:
                continue
            if not self._q4_chase_now(ch, next_cover):
                continue
            self.chase(ch)
            self._q4_path_sensor_scan()
            self._service_certified_nearby(max_walk=NEARBY_CERT_M)
            self._q4_prune_scouts()

    def _q4_shield_insert(
        self, planned: tuple[float, float] | None
    ) -> tuple[float, float] | None:
        """If planned remaining scans would leave some (g,u) unheard, insert lattice."""
        leftover = self._q4_leftover_samples()
        if not leftover:
            return planned
        lattice = q4_scouts()
        remaining_idx = {self._q4_idx(s) for s in self.remaining_scouts}
        stations = list(self.discovery_stations)
        if planned is not None:
            stations.append(planned)
        for g, u, q, certs in leftover:
            heard = any(_pose_hears_gu(s, g, u) for s in stations)
            if heard:
                continue
            covered_later = any(j in remaining_idx for j in certs)
            if covered_later:
                continue
            # Insert nearest unused lattice certificate.
            unused = [
                lattice[j]
                for j in certs
                if not self._q4_scout_done(lattice[j])
            ]
            if not unused:
                unused = [
                    p
                    for p in lattice
                    if not self._q4_scout_done(p)
                ]
            if not unused:
                continue
            here = (self.x, self.y)
            extra = min(unused, key=lambda p: hypot(p, here))
            if extra not in self.remaining_scouts:
                self.remaining_scouts.append(extra)
            return extra
        return planned

    def _run_mixed(self) -> None:
        """SE(2) residual covering salesman on leftover (g,u), lattice shield."""
        self.remaining_scouts = list(q4_scouts())
        self.discovery_stations = []
        self._q4_visited = []
        self._q4_leftover = None
        self.n_path_scans = 0
        ...
        self.n_csp_visits = 0
        self.n_savings_inserts = 0
        while not self._too_many() and self.n_net_visits < 80:
            if not self._silent_channels() or self._n_found() >= N_Q4_MAX:
                break
            self._q4_prune_scouts()
            if not self.remaining_scouts:
                self._q4_certificate_fallback()
            if not self.remaining_scouts:
                if not self._q4_se2_residual_nonempty():
                    break
                self._q4_certificate_fallback()
                if not self.remaining_scouts:
                    break
            nxt = self._q4_next_covering_pose()
            if nxt is None:
                self._q4_certificate_fallback()
                nxt = self._q4_next_covering_pose()
            if nxt is None:
                break
            pose, scout = nxt
            pose_shield = self._q4_shield_insert(pose)
            if pose_shield is not None and scout is not None:
                if hypot(pose_shield, scout) > 40.0 and hypot(pose_shield, pose) > 40.0:
                    pose = pose_shield
                    scout = pose_shield
                    self.n_csp_visits += 0
            lattice_hit = scout is not None and hypot(pose, scout) <= 25.0
            if scout is not None and hypot(pose, scout) > 40.0:
                self.n_csp_visits += 1
            self.n_net_visits += 1
            self._q4_batch_scan(pose, lattice=True)
            if scout is not None and not any(hypot(scout, v) <= 1e-6 for v in self._q4_visited):
                self._q4_visited.append(scout)
            self._q4_prune_scouts()
            next_cover = None
            if self.remaining_scouts:
                follow = self._q4_next_covering_pose()
                if follow is not None:
                    next_cover = follow[0]
            self._q4_chase_useful(next_cover)
        self._service_pending_tsp()
        self._chase_open_fixes()
